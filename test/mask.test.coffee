...
describe "mask", ->
  ...
  it "zipped layer mask", () ->
    psdPath = path.resolve(__dirname, "../", "examples/mask/layer_mask_zipped.psd")
    PSD
    .open(psdPath)
    .then (psd) ->
      { image } = psd.layers[0]
  ...
  it "zipped real user mask", () ->
    psdPath = path.resolve(__dirname, "../", "examples/mask/user_vector_mask_zipped.psd")
    PSD
    .open(psdPath)
    .then (psd) ->
      { image } = psd.layers[0]
